...
def split_decay(module):
    decay, no_decay = [], []
    for m in module.modules():
        for name, p in m.named_parameters(recurse=False):
            if not p.requires_grad: 
                continue
            if name.endswith('bias') or isinstance(m, NORM_LAYERS):
                no_decay.append(p)
            else:
                decay.append(p)
...
# Backbone groups
    bb_decay, bb_nodecay = split_decay(model.backbone)
    param_groups = [
    {'params': bb_decay,   'lr': base_lr, 'weight_decay': 1e-4, 'name': 'bb_decay'},
    {'params': bb_nodecay, 'lr': base_lr, 'weight_decay': 0.0,  'name': 'bb_nodecay'}]

# Head groups (start at base_lr; scheduler will change after warmup)
    for t, head in model.heads.items():
        h_decay, h_nodecay = split_decay(head)
        param_groups += [
            {'params': h_decay,   'lr': base_lr, 'weight_decay': 1e-4, 'name': f'head:{t}:decay'},
            {'params': h_nodecay, 'lr': base_lr, 'weight_decay': 0.0,  'name': f'head:{t}:nodecay'},
        ]

    opt = torch.optim.AdamW(param_groups)
    WARMUP_EPOCHS = 2

# build a per‑group lambda so only heads change after warmup
    lambdas = []
    for g in opt.param_groups:
        name = g.get('name', '')
        if name.startswith('bb_'):
            lambdas.append(lambda ep, _n=name: 1.0)  # backbone constant
        elif name.startswith('head:'):
            # parse task name out of "head:{task}:decay"
            task = name.split(':')[1]
            mult = HEAD_MULT_AFTER[task]
            lambdas.append(lambda ep, m=mult: 1.0 if ep < WARMUP_EPOCHS else m)
        else:
            lambdas.append(lambda ep: 1.0)
    ...
    for ep in tqdm(range(1, args.epochs+1),colour='green'):
        ...
        if ep%10 ==0:
            torch.save(model.state_dict(), os.path.join(args.out_dir, f'tongue_Ep{ep}_full_data.pt'))
...
def main():
    ...
    if args.full:
        # Build the same dataset type your training loop expects for TRAIN.
        if args.tasks == 'tongue':
            base_ds = MultiViewTongueDataset(args.csv, args.img_root,
                                             is_train=True, img_size=224, ra_prob=0.0)
        else:
            base_ds = get_dataset(args)
